...
namespace AimsharpWow.Modules
{
    public class VengeanceShadowlands : Rotation
    {
        ...
        List<string> VengeanceSpells = new List<string>{
            "Demon Spikes","Fiery Brand","Immolation Aura","Fracture","Soul Cleave","Sigil of Flame","Throw Glaive","Metamorphosis",
            "Fel Devastation","Shear", "Infernal Strike","Disrupt"
        };
        ...
        public override bool CombatTick()
        {
            ...
            if (Aimsharp.CanCast("Demon Spikes", "player") && Fighting){
            if (!DemonSpikesUp && selfhealth <= 60){
                Aimsharp.Cast("Demon Spikes");
                return true;}
            }

            if (Aimsharp.CanCast("Fiery Brand", "player") && FieryBrandCDReady && Fighting){
            ...
            if (Venthyr && Aimsharp.CanCast("Sinful Brand") && Fighting && !SinfulDebuffUp){
            ...
            if (FieryBrandCDRemaining >= 10000 || DebuffFieryBrandUp){
                Aimsharp.Cast("Immolation Aura");
                return true;}
            }

            if (Aimsharp.CanCast("Fracture", "player") && Fighting){
            if (MetaUp && Fury <= 55 || !MetaUp && Fury <= 70){
